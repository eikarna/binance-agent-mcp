import { type BinanceConfig, getBaseUrl } from "./config.ts";
...
export class BinanceClient {
	...
	private async signQuery(
		...
		if (!this.config.apiSecret) {
			throw new Error(
				"BINANCE_API_SECRET is required for authenticated endpoints.",
			);
		}
	...
	async cancelOrder(
		symbol: string,
		orderId: number,
	): Promise<{ symbol: string; orderId: number; status: string }> {
		const cleanSymbol = symbol.toUpperCase().replace(/[-_/]/g, "");
