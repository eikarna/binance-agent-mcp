  ...
  public async executeSafeTrade(req: AgentTradeRequest): Promise<any> {
    ...
    if (cached) {
      if (cached.state === 'EXECUTED') {
        return { success: true, status: 'IDEMPOTENT_REPLAY', clientOrderId, data: cached.response };
      }
      if (cached.state === 'PENDING') {
        throw new Error(`Duplicate execution in progress for clientOrderId: ${clientOrderId}`);
      }
    }
    ...
    const normalizedQty = this.precision.roundToStep(req.quantity, filters.stepSize);
    const normalizedPrice = req.price ? this.precision.roundToTick(req.price, filters.tickSize) : undefined;
    ...
    if (!policyResult.allowed) {
      this.idempotency.recordResult(clientOrderId, 'REJECTED', { reason: policyResult.reason });
    ...
    try {
      ...
      this.idempotency.recordResult(clientOrderId, 'REJECTED', { error: err.message });
      throw err;
    }
  }
