...
export class StrategyAgent {
  ...
  public async start() {
    ...
    console.log(`Threshold: +/-${this.config.imbalanceThreshold} | Interval: ${this.config.checkIntervalMs}ms | Qty: ${this.config.tradeQuantity}`);
  ...
  private async loop() {
    ...
    if (this.isRunning) {
      this.timeoutId = setTimeout(() => this.loop(), this.config.checkIntervalMs);
    }
  }
  ...
  private async evaluate() {
    ...
    console.log(`[${new Date().toISOString()}] ${this.config.symbol} | Price: $${currentPrice.toFixed(2)} | Spread: ${ob.spreadPercent}% | Imbalance: ${imbalance.toFixed(4)}`);

    if (imbalance >= this.config.imbalanceThreshold) {
      console.log(`🔥 HIGH BUY IMBALANCE DETECTED (${imbalance.toFixed(4)}). Executing BUY.`);
      await this.executeTrade("BUY");
    } else if (imbalance <= -this.config.imbalanceThreshold) {
      console.log(`🩸 HIGH SELL IMBALANCE DETECTED (${imbalance.toFixed(4)}). Executing SELL.`);
  ...
  private async executeTrade(side: "BUY" | "SELL") {
    try {
      ...
      console.log(`✅ ${side} Order Executed! ID: ${order.orderId} | Status: ${order.status}`);
