	...
	public validateTrade(
		...
		if (this.config.isKillSwitchActive) {
			return {
				allowed: false,
				reason:
					"CRITICAL: Emergency Kill Switch is engaged. All trading is halted.",
				notionalUsd: 0,
			};
		}
		...
		if (!effectivePrice || effectivePrice <= 0) {
			return {
				allowed: false,
				reason:
					"INVALID_PRICE: Trade price or reference mark price must be strictly positive.",
				notionalUsd: 0,
			};
		}
		...
		if (request.quantity <= 0) {
			return {
				allowed: false,
				reason:
					"INVALID_QUANTITY: Order quantity must be strictly greater than zero.",
				notionalUsd: 0,
			};
		}
		...
		if (
			...
			const priceDiffBps =
				Math.abs(
					(request.price - request.referencePrice) / request.referencePrice,
				) * 10000;
