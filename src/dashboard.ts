...
export class Dashboard {
  ...
  public async start() {
    ...
    while (this.isRunning) {
      ...
      try {
        ...
        this.log(`API Error: ${err instanceof Error ? err.message : String(err)}`, "error");
      }
    }
  }
  ...
  private randomizeMockData() {
     ...
     this.bids = this.bids.map(b => [b[0] + (Math.random() - 0.5)*10, b[1] * (1 + (Math.random() - 0.5)*0.1)]);
     this.asks = this.asks.map(a => [a[0] + (Math.random() - 0.5)*10, a[1] * (1 + (Math.random() - 0.5)*0.1)]);
  ...
  private async fetchData() {
    ...
    if (this.imbalance > 0.4) {
      this.log(`STRONG BUY SIGNAL: Imbalance +${(this.imbalance*100).toFixed(1)}%`, "buy");
    } else if (this.imbalance < -0.4) {
      this.log(`STRONG SELL SIGNAL: Imbalance ${(this.imbalance*100).toFixed(1)}%`, "sell");
    }
  }
  ...
  private renderDepthGraph(): string {
    ...
    if (maxVol === 0 || !Number.isFinite(maxVol)) return "Awaiting orderbook...";
