...
function encodeType(primaryType: string, types: EIP712Types): string {
  ...
  const deps = Array.from(typeSet).filter((t) => t !== primaryType).sort();
...
function encodeDataValue(type: string, value: any): Uint8Array {
  ...
  if (type === "string" || type === "bytes") {
    if (typeof value === "string") {
      const bytes = type === "bytes" && value.startsWith("0x") 
        ? hexToBytes(value)
...
function encodeData(primaryType: string, data: any, types: EIP712Types): Uint8Array {
...
function hashStruct(primaryType: string, data: any, types: EIP712Types): Uint8Array {
...
export function hashEIP712Message(typedData: EIP712TypedData): Uint8Array {
  ...
  const digest = new Uint8Array(prefix.length + domainHash.length + messageHash.length);
...
export function recoverAddress(
  ...
  const pubKeyPoint = secp256k1.recoverPublicKey(signature.toBytes("recovered"), digest, { prehash: false, isCompressed: false });
