	...
	async (input: string | Request | URL, init?: RequestInit) => {
		...
		if (pathname.includes("/api/v3/ticker/24hr")) {
			...
			if (symbol && symbol in fixtures.ticker) {
				return new Response(
					JSON.stringify(
						fixtures.ticker[symbol as keyof typeof fixtures.ticker],
					),
				);
			}
			return new Response(
		...
		if (pathname.includes("/api/v3/depth")) {
			...
			if (symbol && symbol in fixtures.depth) {
				return new Response(
					JSON.stringify(fixtures.depth[symbol as keyof typeof fixtures.depth]),
				);
			}
			return new Response(
				JSON.stringify({ code: -1121, msg: "Invalid symbol." }),
		...
		if (pathname.includes("/api/v3/order")) {
			...
			if (init?.method === "DELETE") {
				...
				const orderId = parseInt(
					searchParams.get("orderId") || "123456789",
					10,
				);
				return new Response(
					JSON.stringify({ symbol, orderId, status: "CANCELED" }),
				);
			}
		}
